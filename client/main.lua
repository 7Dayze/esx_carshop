...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_carshop:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_carshop:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_carshop:sendCategories')
AddEventHandler('esx_carshop:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('esx_carshop:sendVehicles')
AddEventHandler('esx_carshop:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function OpenShopMenu()
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('esx_carshop: vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				local playerData = ESX.GetPlayerData()
					ESX.TriggerServerCallback('esx_carshop:buyVehicle', function(hasEnoughMoney)
						if hasEnoughMoney then
							...
							ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
								...
								if Config.EnableOwnedVehicles then
									TriggerServerEvent('esx_carshop:setVehicleOwned', vehicleProps)
								end
...
-- Entered Marker
AddEventHandler('esx_carshop:hasEnteredMarker', function (zone)
...
-- Exited Marker
AddEventHandler('esx_carshop:hasExitedMarker', function (zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_carshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_carshop:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction == nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('esx_carshop:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
							ESX.ShowNotification(_U('car_sold_for', CurrentActionData.label, ESX.Math.GroupDigits(CurrentActionData.price)))
						else
